...
public class RedisReactiveOAuth2AuthorizedClientService
  ...
  public <T extends OAuth2AuthorizedClient> Mono<T> loadAuthorizedClient(
    ...
    OAuth2AuthorizedClientId authorizedClientId =
    ...
    return (Mono<T>)
        redisOperations
            ...
            .doOnSuccess(
                c -> log.debug("Load authorized client {} for principal {}", c, principalName))
            .doOnError(
                e ->
                    log.error(
                        "Failed to load authorized client for principal {}", principalName, e));
  }
  ...
  public Mono<Void> saveAuthorizedClient(
    ...
    OAuth2AuthorizedClientId authorizedClientId =
        new OAuth2AuthorizedClientId(registrationId, principalName);
    return redisOperations
        ...
        .doOnNext(
            b ->
                log.debug(
                    "Save authorized client {} for principal {}", authorizedClient, principal))
        .doOnError(
            e ->
                log.error(
                    "Failed to save authorized client {} for principal {}",
                    authorizedClient,
                    principal,
                    e))
        .then();
  ...
  public Mono<Void> removeAuthorizedClient(String clientRegistrationId, String principalName) {
    ...
    return redisOperations
        ...
        .doOnNext(
            b ->
                log.debug(
                    "Remove authorized client {} for principal {}",
                    clientRegistrationId,
                    principalName))
        .doOnError(
            e ->
                log.error(
                    "Failed to remove authorized client {} for principal {}",
                    clientRegistrationId,
                    principalName,
                    e))
        .then();

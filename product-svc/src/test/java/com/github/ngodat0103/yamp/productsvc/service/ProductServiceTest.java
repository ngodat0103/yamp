...
class ProductServiceTest {
  ...
  void setUp() {
    ...
    this.productService =
        new ProductServiceImpl(productRepository, categoryRepository, slugify, productMapper);
  ...
  @DisplayName(
      "Given a ProductDto, when creating a product, then throw ConflictException if the product name already exists")
  public void givenProductDto_whenCreateProduct_thenThrowConflictException() {
    ...
    String expectedMessage =
        String.format("Product with name: \"%s\" already exists", productDtoRequest.getName());
  ...
  @CustomerTestWithMockUser
  @DisplayName(
      "Given a ProductDto, when creating a product, then throw NotFoundException if the category does not exist")
  public void givenProductDto_whenCreateProduct_thenThrowNotFoundException() {
    ...
    Assertions.assertEquals(
        "Category with slugName: " + productDtoRequest.getCategorySlug() + " not found",
  ...
  public void givenProductDto_whenCreateProduct_thenReturnProductDto() {
    ...
    given(categoryRepository.findCategoryBySlugName(this.productDtoRequest.getCategorySlug()))
        .willReturn(java.util.Optional.of(category));
  ...
  @CustomerTestWithMockUser
  @DisplayName(
      "Given a ProductDto, when updating a product, then throw NotFoundException if the product does not exist")
  public void givenProductDto_whenUpdateProduct_thenThrowNotFoundException() {
    ...
    Assertions.assertEquals(
        "Product with uuid: " + product.getUuid() + " not found", exception.getMessage());
  }

  @CustomerTestWithMockUser
  @DisplayName(
      "Given a ProductDto, when updating a product, then throw NotFoundException if the category does not exist")
  ...
  public void givenProductDto_whenUpdateProduct_thenReturnProductDto() {
    ...
    ProductDtoResponse productDtoResponse =
        productService.updateProduct(product.getUuid(), this.productDtoRequest);
    Assertions.assertNotNull(productDtoResponse);
  ...
  @Test
  @DisplayName(
      "Given a product UUID, when deleting a product, then throw NotFoundException if the product does not exist")
  ...
  @Test
  @DisplayName(
      "Given productSlug, when get product by slugName, then throw NotFoundException if the product does not exist")
  void givenProductSlug_whenGetProductBySlug_thenThrowNotFoundException() {
    ...
    Assertions.assertEquals(
        "Product with slugName: " + product.getSlugName() + " not found", exception.getMessage());
  }
  ...
  void givenProductSlug_whenGetProductBySlug_thenReturnProductDto() {
    ...
    Assertions.assertEquals(
        product.getCategory().getSlugName(), productDtoResponse.getCategorySlug());
  ...
  @Test
  @DisplayName(
      "Given product UUID, when get product by UUID, then throw NotFoundException if the product does not exist")
    ...
    pageDto
        ...
        .forEach(
            productDto -> {
              ...
              Assertions.assertEquals(
                  product.getCategory().getSlugName(), productDto.getCategorySlug());
